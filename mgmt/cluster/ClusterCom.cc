...
ClusterCom::base_sendReliableMessage(unsigned long addr, const char *buf, int len, bool take_lock)
{
  ...
  if ((fd = mgmt_socket(AF_INET, SOCK_STREAM, 0)) < 0) {
    mgmt_elog(errno, "[ClusterCom::base_sendReliableMessage] Unable to create socket\n");
    return false;
  }
...
error:
        mgmt_elog(0, "[ClusterCom::base_sendReliableMessage] Unable to connect to peer\n");
        close_socket(fd);
        return false;
      }
    }
  ...
  if (fcntl(fd, F_SETFL, 1) < 0) {
    mgmt_elog(errno, "[ClusterCom::base_sendReliableMessage] Unable to unset O_NONBLOCK.\n");
    close_socket(fd);
    return false;
  }

  if (mgmt_writeline(fd, buf, len) != 0) {
    mgmt_elog(stderr, errno, "[ClusterCom::base_sendReliableMessage] Write failed\n");
    close_socket(fd);
    return false;
  }
  return fd;
