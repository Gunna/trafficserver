...
update_real_stats(HttpSM *sm)
{
  ...
  rt = (sm->milestones.ua_close - sm->milestones.ua_begin) / 1000;
...
HttpSM::state_add_to_list(int event, void *data)
{
  ...
  if (t_state.http_config_param->enable_http_info) {
    ...
    // FIX ME: the sm_list may miss some http_sms when the lock contention
    if (lock)
  ...
  case CACHE_EVENT_OPEN_READ:
    {
      ...
      t_state.cache_info.is_ram_cache_hit =
        t_state.http_config_param->record_tcp_mem_hit && (cache_sm.cache_read_vc)->is_ram_cache_hit();
      t_state.cache_info.is_read_from_writer = (cache_sm.cache_read_vc)->is_read_from_writer();
...
HttpSM::is_private()
{
   HttpServerSession * ss = server_session ? server_session : ua_session->get_server_session();
